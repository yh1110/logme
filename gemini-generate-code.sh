...
on_error() {
  echo "エラーが発生しました。ブランチとPRを削除します。"
  # PRクローズ＆ブランチ削除
  if [[ -n "${PR_NUMBER:-}" ]]; then
    gh pr close "$PR_NUMBER" --delete-branch --repo "$REPO" || true
  fi
...
if [ -z "$ISSUE_NUMBERS" ]; then
  echo "'gemini'のlabelが付いたオープンなIssueはありません。"
  exit 0
fi
...
for ISSUE_NUMBER in $ISSUE_NUMBERS; do
  ...
  # ——————————————
  # ブランチ名生成 例：gemini/feature/#123
  # ——————————————
  BRANCH_NAME="gemini/feature/${ISSUE_NUMBER}"
  ...
  # gemini に投げる最初のプロンプト（非対話モード）
  PROMPT_PATCH="$(printf '%s\n\n%s\n%s\n' \
  "GitHub Issue #$ISSUE_NUMBER: $ISSUE_TITLE" \
  "$ISSUE_BODY" \
  "unified diff（パッチ）のみを返してください。'.git' のような'.'から始まるファイル、フォルダの中身は絶対に変更しないでください。")"

  # 1) gemini へプロンプトを投げてパッチを変数にキャプチャ

  PROMPT=$(printf '%s\n\n%s\n\n%s\n' \
    "GitHub Issue #$ISSUE_NUMBER: $ISSUE_TITLE" \
    "$ISSUE_BODY" \
    "unified diff（diff --git a/... b/... 形式）のみを返してください。")
  PATCH_RAW=$(gemini -p "$PROMPT" --yolo)


  # # 2) まずファイルに書き出して中身を確認
  # echo "$PATCH_RAW" > /tmp/gemini.patch
  # echo ">>> Generated patch (head) <<<"
  # head -n 20 /tmp/gemini.patch
  # echo ">>> (…以上先頭20行) <<<"

  # PATCH_CLEAN=$(sed -n '/^diff --git/,${p}' /tmp/gemini.patch)
  ...
  SUMMARY=$(gemini -p "$PROMPT_SUMMARY" --yolo)
